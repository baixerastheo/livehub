...
export class ReactionService {
  ...
  private validateEmoji(emoji: string): void {
    if (!emoji?.trim()) throw new BadRequestException('Emoji requis');
  }

  /**
   * Récupère les réactions d'un message de canal, les agrège et émet l'événement temps réel.
   */
  private async fetchChannelAggregateAndEmit(
    messageId: number,
    canalId: number,
  ): Promise<ReactionAggregate[]> {
   ...
   */
  private async fetchPrivateAggregateAndEmit(
    messageId: number,
    context: { expediteurId: string; destinataireId: string },
  ): Promise<ReactionAggregate[]> {
   ...
   */
  private async toggleChannelRecord(
    messageId: number,
    userId: string,
    emoji: string,
  ): Promise<void> {
   ...
   * Supprime la réaction si elle existe, la crée sinon.
   */
  private async togglePrivateRecord(
    messageId: number,
    userId: string,
    emoji: string,
  ): Promise<void> {
