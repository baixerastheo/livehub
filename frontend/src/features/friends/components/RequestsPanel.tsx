...
export function RequestsPanel() {
  ...
  if (requestsQuery.isLoading) return <div className={styles.state}>Loading requests…</div>;
              ...
              <li key={r.id} className={styles.row}>
                <div className={styles.meta}>
                  <div className={styles.avatar} aria-hidden="true">
                    {r.fromUser.nomUtilisateur?.slice(0, 1)?.toUpperCase() ?? "?"}
                  </div>
                  <div className={styles.text}>
                    <div className={styles.username}>{r.fromUser.nomUtilisateur}</div>
          ...
          <ul className={styles.list}>
            {outgoing.map((r) => (
              <li key={r.id} className={styles.row}>
                <div className={styles.meta}>
                  <div className={styles.avatar} aria-hidden="true">
                    {r.toUser.nomUtilisateur?.slice(0, 1)?.toUpperCase() ?? "?"}
                  </div>
                  <div className={styles.text}>
                    <div className={styles.username}>{r.toUser.nomUtilisateur}</div>
                    <div className={styles.subtleRow}>pending</div>
                  </div>
                </div>
                <div className={styles.actions}>
